...
class ContactController {
  async findUsersContact(req, res) {
    try {
      ...
      let users = await contactService.findUsersContact(
        currentUserId,
        keyword
      );
  ...
  async addNew(req, res) {
    try {
      ...
      let newContact = await contactService.addNew(
        currentUserId,
        contactId
      );
  ...
  async removeRequestContactReceived(req, res) {
    try {
      ...
      let contactId = req.body.uid;
      let removeReq =
        await contactService.removeRequestContactReceived(
          currentUserId,
          contactId
        );
      return res.status(200).json({ success: !!removeReq });
  ...
  async readMoreContactsSent(req, res) {
    try {
      let skipNumberContacts = +req.query.skipNumber;
      let newContactUsers =
        await contactService.readMoreContactsSent(
          req.user._id,
          skipNumberContacts
        );
      return res.status(200).json(newContactUsers);
  ...
  async readMoreContactsReceived(req, res) {
    try {
      let skipNumberContacts = +req.query.skipNumber;
      let newContactUsers =
        await contactService.readMoreContactsReceived(
          req.user._id,
          skipNumberContacts
        );
      return res.status(200).json(newContactUsers);
