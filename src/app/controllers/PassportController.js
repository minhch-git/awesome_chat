  ...
  applyPassportFacebook(passport) {
    passport.use(new FacebookStrategy({
      ...
      try {
        ...
        if (user) return done(null, user, req.flash('success', tranSuccess.login_success(profile.displayName)))
        const userItem = {
        ...
        done(null, newUser, req.flash('success', tranSuccess.login_success(newUser.username)))
      } catch (error) {
        done(error, null, req.flash('error', req.flash('errors', transErrors.login_failed)))
      }
    }))
  }
  ...
  applyPassportGoogle(passport) {
    passport.use(new GoogleStrategy({
      ...
      try {
        ...
        if (user) return done(null, user, req.flash('success', tranSuccess.login_success(user.username)))
        const userItem = {
        ...
        const newUser = await User.createNew(userItem)
        return done(null, newUser, req.flash('success', tranSuccess.login_success(newUser.username)))
      } catch (error) {
        done(error)
      }
    }))
